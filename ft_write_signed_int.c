...
/*   Updated: 2017/01/04 14:05:31 by cfatrane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_write_justify_precision_signed_int(t_env *arg, long long int nbr)
{
	...
	if (nbr >= 0)
	{
		if (arg->size > arg->precision.nbr/* && arg->precision.nbr > arg->len*/)
		{
			...
			ft_putnbr(nbr);
			i = 0;
			while (i < arg->size - arg->precision.nbr - lenfin)
			{
				ft_putchar (' ');
				i++;
			}
	...
	{
		ft_putchar('-');
		i += ft_write_flag_zero(arg->precision.nbr, arg->len + 1);
		ft_putnbr(ft_abs(nbr));
...
static int	ft_write_flag_precision(t_env *arg, long long int nbr)
{
	...
	else
	{
		...
		else
		{
			if (arg->size > arg->precision.nbr/* && arg->precision.nbr > arg->len*/)
			{
				i += ft_write_flag_spaces(arg->size, arg->precision.nbr + 1);
				ft_putchar('-');
				i = 0;
				i += ft_write_flag_zero(arg->precision.nbr, arg->len - 1);
				ft_putnbr(ft_abs(nbr));
			...
			{
				ft_putchar('-');
				arg->len += ft_write_flag_zero(arg->precision.nbr, arg->len - 1);
				ft_putnbr(ft_abs(nbr));
				arg->len += i + lenfin;
				return (ft_nbcmp(arg->len, arg->precision.nbr));
			}
	...
	//	printf("len = %d||\n prec = %d||\n size = %d||\n\n", arg->len, arg->precision.nbr, arg->size);
	//	return (ft_nbcmp(ft_nbcmp(arg->precision.nbr, arg->len), ft_nbcmp(arg->precision.nbr, arg->size)));
	return (0);
}
...
int	ft_write_signed_int(t_env *arg, va_list ap)
{
	...
	if ((nbr >= 0) && (arg->flags.options[MORE] == 1 || arg->flags.options[SPACE] == 1) && (!arg->flags.options[LESS]) && ((!arg->size) && (arg->precision.actif == 0)))
